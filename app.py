...
st.caption("단기유동성으로 금융시장 참여자들(딜러, 헤지펀드)이 체감.즉, 미래에 사용할 수 있는 '연료'가 줄어들고 있음을 미리 보여줍니다.")
...
# 5-2. Gemini 챗봇 함수
# ============================================================
def generate_gemini_chat_response(df, assessment, user_question, history):
    """유동성 지표 맥락 + 이전 대화 기반 챗봇 응답"""
    if not GEMINI_AVAILABLE:
        return "⚠️ Gemini API 키가 없어 챗봇 기능을 사용할 수 없습니다."
    ...
    prompt = f"""
당신은 연준 유동성, Dark Pool, Short Interest 및 거시·시장 분석 전문가입니다.
친절하고 실용적인 조언을 한국어로 제공하세요.

## 현재 유동성 요약 (최신 날짜: {assessment['latest_date']})
- 종합 상태: {overall['status']} (점수 {overall['score']}/{overall['max_score']})
- 평가: {overall['message']}
- RP: {assessment['latest_values']['RP']:.2f}B
- RRP: {assessment['latest_values']['RRP']:.2f}B
- 은행 지준금: {assessment['latest_values']['Reserves']:.2f}B
- SOFR-IORB 스프레드: {assessment['latest_values']['Spread']:.2f}bps
...
위 정보를 참고하여,
1. 사용자의 질문을 먼저 한 문장으로 재정리하고,
2. 핵심 답변을 3~6문장 정도로 설명하고,
3. 필요하다면 Bullet 형식으로 리스크/체크 포인트를 정리해 주세요.

투자 종목을 직접 추천하지 말고, 원칙과 프레임 위주로 설명해 주세요.
"""
    ...
    try:
        ...
        if "quota" in error_msg.lower() or "429" in error_msg:
            return "⚠️ API 할당량 초과. 잠시 후 다시 시도하세요."
        return f"⚠️ 챗봇 응답 생성 중 오류 발생: {error_msg}"
...
# ============================================================
def main():
    #st.title("🏦 연준 유동성 스트레스 모니터링 대시보드")
    #st.markdown("---")
    ...
    # 탭
    st.markdown("---")
    tab1, tab2, tab3, tab4 = st.tabs(["📊 개별 지표 분석", "📈 이동평균 교차", "🤖 AI 분석 & 챗봇", "📖 해석 가이드"])
    ...
    with tab3:
        ...
        st.caption("💡 연준 유동성, Dark Pool, Short Interest, 매크로·시장 전략 등에 대해 자유롭게 질문해 보세요.")
        ...
        if st.session_state["chat_history"]:
            ...
            st.info("아직 대화가 없습니다. 아래에 질문을 입력해 보세요.")
        ...
        with col_chat1:
            user_question = st.text_area(
                "질문 입력",
                key="chat_input",
                placeholder="예: 지금 같은 유동성 환경에서 성장주 vs 가치주 중 어느 쪽이 상대적으로 유리한지 알려줘.",
                height=80
            )
        with col_chat2:
            st.write("")
        ...
        if send_btn and user_question.strip():
            ...
            with st.spinner("🤖 AI가 생각 중입니다..."):
                answer = generate_gemini_chat_response(df, assessment, user_question.strip(), st.session_state["chat_history"])
...
"""
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; padding: 20px;'>
            <p>🏦 연준 유동성 스트레스 모니터링 대시보드 v2.1</p>
            <p>데이터 출처: FRED (Federal Reserve Economic Data) | AI: Gemini 2.5 Flash</p>
